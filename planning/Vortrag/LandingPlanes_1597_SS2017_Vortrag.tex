...
\begin{frame} {posix Threads}{Standard - was gehört dazu}
	...
	\note{
		\begin{itemize}
			\item typisch: pthread_create, pthread_exit, pthread_cancel, pthread_join , pthread_detach
			\item mutex: pthread_mutex_init, pthread_mutex_destroy, pthread_mutex_lock, pthread_mutex_trylock, pthread_mutex_unlock 
			\item condition variables: pthread_cond_init, pthread_cond_destroy, werden benutzt um Threadverhalten in Datenabhängigkeit zu steuern (ohne teures Polling)
			\item Empfehlung: Kurs 01727 Mitautor anwesend 
			\was nicht inbegriffen sind Semaphoren. Eine Art Mutex, die allerdings einen Zähler beinhaltet. Mutex ist quasi ein binäres Semaphor
		\end{itemize}
	}
...
\begin{frame}{Speedup in Abhängigkeit des Parallelisierungsgrades}{Messverfahren}
			...
			\item pro Parameterset 4 unabhängige Läuft (Mittelwert + Standardabweichung)
...
\begin{frame}{Laufzeitanalyse mittels Profiler}
    		...
    		\item größter Laufzeitanteil für die Zugriffsfunktion (Random Memory acceess)
