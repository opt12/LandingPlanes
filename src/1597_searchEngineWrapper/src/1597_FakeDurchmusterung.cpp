...
void *createFakePlaneThreadStarter(void *par) {
	...
	search_for_planes(p.actualTile, p.myGeoTiffHandler, p.actualHeading,
			(*p.taskDescription)["scanParameters"]["minLength"],
			(*p.taskDescription)["scanParameters"]["minWidth"], p.commSocket,
			p.taskDescription, p.noDataValue, p.pixelSize,
