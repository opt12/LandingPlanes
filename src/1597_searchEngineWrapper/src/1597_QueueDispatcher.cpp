...
static eResult saveDataToMFile(json taskDescription){
	...
	geoCoord geoTopLeft = {taskDescription["mapExtent"]["topLeftLat"],  taskDescription["mapExtent"]["topLeftLng"]};
	geoCoord geoBotRight = {taskDescription["mapExtent"]["botRightLat"],  taskDescription["mapExtent"]["botRightLng"]};

	tilingCharacteristics myTilingChar;
	myGeoTiffHandler.getTilingInfo(geoTopLeft, geoBotRight, 2000, MAX_SIZE, &myTilingChar);
	...
	for(int idxX=0; idxX< myTilingChar.tilesInX; idxX++){
		for(int idxY=0; idxY< myTilingChar.tilesInY; idxY++){
			...
			cout << tile;
			string fileNameOut= taskDescription["mFileName"].get<std::string>();
			printMatrixToFile(fileNameOut.c_str(), fileNameOut.c_str(),
...
eResult Dispatcher(void) {
	...
	do {
		...
		json queueHead = taskQueue->front();
		taskQueue->pop();

		currentTask={convertTaskType(queueHead["type"].get<std::string>()), queueHead["data"]};
		...
		switch (currentTask.taskType) {
			case SCAN:
				cout << "Dispatcher Thread: dispatching task to scanForLandingPlanes()"<<endl;
				cout << currentTask.taskDescription.dump(4) << endl;
				scanForLandingPlanes(currentTask.taskDescription);
				break;
			case SAVE_2_M_FILE:
				cout << "Dispatcher Thread: dispatching task to saveDataToMFile()"<<endl;
				cout << currentTask.taskDescription.dump(4) << endl;
				saveDataToMFile(currentTask.taskDescription);
				break;
			default:
				cout << R"(Dispatcher Thread: ignoring unknown taskType: )" << queueHead["type"] << endl;
				cout << currentTask.taskDescription.dump(4) << endl;
				break;
		}
