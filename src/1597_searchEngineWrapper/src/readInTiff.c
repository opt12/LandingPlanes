...
int getImageInformation(tileCharacteristics* info, const char * infilename) {
	...
	info->memsize = computeMemorySize(info->spp, info->bitspersample,
			info->outwidth, info->outlength);
...
//	void * out; /* TIFF* or FILE* */
	...
	//the memory is already preallocated at position tile->buf in memory
	//This is handled outside the extraction routine;
